...
{%- for post in posts -%}
  ...
  {%- assign shell = 'python3 share-card-creator.py ' -%}

  {% comment %} Add the title to the shell command {% endcomment %}
  {%- assign title = post.title -%}
  {%- assign shell = shell | append: ' -' | append: '-title="' | append: title | append: '" ' -%}
  ...
  {%- for guest-detail in guest-details -%}
    ...
    {%- if index-id == total-guests -%}
      ...
      {%- assign shell = shell | append: ' --card_file="' | append: card-filename | append: '"' -%}
{% capture final-shell %}
{{ shell }}
{% endcapture %}
{{ final-shell }}
